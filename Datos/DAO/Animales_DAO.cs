...
namespace Proyecto_Zoologico.Datos.DAO
{
    public class Animales_DAO
    {
        ...
        public void AgregarAnimal(Animales animal)
        {
            try
            {
                string query = "INSERT INTO Tb_Animales (Animal_Especie, Animal_Tamaño, Animal_Peso, Animal_Dieta, Alimentacion_Id) " +
                               "VALUES (@Especie, @Tamaño, @Peso, @Dieta, @AlimentacionId)";
                ...
                cmd.Parameters.AddWithValue("@AlimentacionId", animal.Alimentacion_Id);
                conexion.Open();
                cmd.ExecuteNonQuery();
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    Animales animal = new Animales(
                        ...
                        Convert.ToInt32(reader["Alimentacion_Id"])
                    );
        ...
        public void ActualizarAnimal(Animales animal)
        {
            try
            {
                string query = "UPDATE Tb_Animales SET Animal_Especie = @Especie, Animal_Tamaño = @Tamaño, " +
                               "Animal_Peso = @Peso, Animal_Dieta = @Dieta, Alimentacion_Id = @AlimentacionId WHERE Animal_Id = @Id";
                MySqlCommand cmd = new MySqlCommand(query, conexion);
                cmd.Parameters.AddWithValue("@Especie", animal.Animal_Especie);
                cmd.Parameters.AddWithValue("@Tamaño", animal.Animal_Tamaño);
                cmd.Parameters.AddWithValue("@Peso", animal.Animal_Peso);
